...
app = Flask(__name__)
app.secret_key = 'social18_secret_key_change_me'
...
def register():
    ...
    cursor = conn.cursor()
    cursor.execute('''
        INSERT INTO registrations (full_name, email, age, phone, sport)
        VALUES (?, ?, ?, ?, ?)
    ''', (full_name, email, age, phone, sport))
    conn.commit()
    conn.close()

    flash("Registration Successful! See you at the arena.", "success")
    return redirect(url_for('home'))

# Private Admin Route to view all attendees
@app.route('/admin-data')
def admin_data():
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute('SELECT id, full_name, email, age, phone, sport, created_at FROM registrations ORDER BY id DESC')
